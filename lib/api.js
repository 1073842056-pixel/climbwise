...
const API_BASE = 'https://api.minimax.chat/v1/text/chatcompletion_pro?GroupId=minimax';
...
async function analyzeRoute(imageBase64) {
  try {
    ...
    if (data.choices && data.choices[0]) {
      const msg = data.choices[0].messages || data.choices[0];
  ...
  try {
    const response = await fetch(API_BASE, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: '你是一个专业攀岩教练，给出简短、鼓励性的建议（50字以内）。' },
