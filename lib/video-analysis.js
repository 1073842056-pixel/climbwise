...
function buildAnalysisPrompt(frames, savedBeta, profile) {
  ...
  const betaText = savedBeta 
    ? `这条线路的最优beta：\n${savedBeta.beta.map(b => `${b.step}. ${b.description}（${b.moveType||'mixed'}）`).join('\n')}`
    : '（用户未保存该线路的读线结果）';
...
2. 识别用户实际使用的动作类型（Campusing/DropKnee/HeelHook/Smear等）
3. 评估每步动作的质量（1-10分）+ 给出问题分析
4. 对比用户动作和最优beta的差异
5. 结合用户弱点给出针对性训练建议
...
**返回严格JSON格式（不要任何其他文字）：**
{
  ...
  "overallScore": 7.2,
  "stepScores": [
    {"step":1,"score":8,"description":"起始动作标准","issue":null,"suggestion":null,"moveType":"OpenCrimp"},
    {"step":2,"score":6,"description":"右手抓点时身体有飘动","issue":"核心没收紧导致身体偏摆","suggestion":"多练1分钟plank","moveType":"Crimp"}
  ],
  "mainIssues": ["核心力量不足导致第三步身体飘","换脚犹豫约1秒"],
  "improvements": ["每天3分钟plank（目标90秒）","专注第三步核心收紧","Hip Hinge专项训练"],
  "detectedBeta": "beta2",
  "betaMatchRate": "65%",
  "detectedMoves": ["OpenCrimp","DropKnee","Sloper","Pinch"],
  "trainingPlan": ["每日3分钟plank","换脚专项练习（目标0.5秒）","每周2次指力板"]
}`
    }],
