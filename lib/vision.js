...
- **禁止**：泛泛描述如"核心收紧"、"重心转移"——必须具体到哪个手哪只脚
- **禁止**：孤立描述手或脚——必须同步描述
- **禁止**：handPosition和footPosition描述相同——它们是不同维度
- **禁止**：keyPoint说"保持稳定"而不说如何判断稳定
- **禁止**：随意编造坐标——每个x,y必须对应岩壁上真实可见的位置
- **禁止**：没有难点的Beta——每条线路必须有cruxStep=true的一步
...
function buildFallbackResult(color) {
  return {
    ...
    weightScore: 9.0,
    personalScore: 8.0,
    subjectiveGrade: '对你来说约等于 V3（演示数据）',
